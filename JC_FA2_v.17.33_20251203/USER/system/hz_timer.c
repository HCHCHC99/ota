...
void nbDelay_Init(NonBlockingDelay_t* delayObj, uint64_t delayMs) {
    ...
    if (delayMs < NB_DELAY_MIN_MS || delayMs > NB_DELAY_MAX_MS) {
        delayMs = NB_DELAY_MIN_MS;  // 非法值修正为最小值（或返回错误，按需设计）
    }
...
void nbDelay_Restart(NonBlockingDelay_t* delayObj)
{
    nbDelay_Start(delayObj);
}
